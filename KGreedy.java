import java.util.*;
class KGreedy extends OptimizationAlgorithm
{
    ...
    public KGreedy(Mylistclass<WBPair> ilist)
    {
        name = this.getClass().getName();
        setInstance(ilist);
    }
    ...
    public void run()
    {
        ...
        //showInstance();
        //report();
        long[] x = new long[pairlist.length];
        
        for (int index = 0; index <= pairlist.length -1 ; index ++)
        {
            x[index] = 0;
             stepcount++;
             maxdepth++;
